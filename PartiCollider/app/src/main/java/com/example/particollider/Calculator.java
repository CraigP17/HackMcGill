...
public class Calculator {
    ...
    public void finalVel(Particle p1, Particle p2){ //final velocity calculator
        ...
        if (p1.isElastic() || p2.isElastic()){ //checking if a particle is inelastic
            ...
            p1vxf=p2vx-((2*p1m)/(p1m+p2m))*(((p2vx-p1vx)*(p2px-p1px))/(f))*(p2px-p1px); //particle 2 final x velocity
            p1vyf=p2vy-((2*p1m)/(p1m+p2m))*(((p2vy-p1vy)*(p2py-p1py))/(f))*(p2py-p1py); //particle 2 final y velocity
        }
    }
